//
//  BreadcrumbPathRenderer.swift
//  CityAdventure
//
//  Created by Pin Chen on 2024/5/12.
//
...
class BreadcrumbPathRenderer: MKOverlayRenderer {
  ...
  override func draw(_ mapRect: MKMapRect, zoomScale: MKZoomScale, in context: CGContext) {
    ...
    if let path = pathForPoints(points, mapRect: clipRect, zoomScale: zoomScale) {
      ...
      context.setStrokeColor(UIColor(hex: "E7F161", alpha: 1).cgColor)
      context.setLineJoin(.round)
      context.setLineCap(.round)
      context.setLineWidth(20)  // 測試renderer寬度
      context.strokePath()
    }
  }
